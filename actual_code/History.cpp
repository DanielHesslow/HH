...
// GOTCHA:
//
// the state of the textbuffer is invalid between we've added/removed the normal caret  but not the selection
// I don't like this and in practice it won't be a problem because of waypoints. We should fix this later tough..
//
...
bool move_forward(History *history, HistoryLocation *location, int branch_index) { //state = -1 -1 = 'before buffer'
...
bool move_backward(History *history, HistoryLocation *location) { //state = -1 -1 = 'before buffer'
...
bool undo_(BackingBuffer *backingBuffer) {
	...
	case op_remove_cursor:
	{
		...
		if (!data.is_selection) {
			instr.textBuffer->ownedCarets_id.add(instr.cursor_id);
			instr.textBuffer->cursorInfo.add({});
		} else {
			instr.textBuffer->ownedSelection_id.add(instr.cursor_id);
		}
	}break;
	case op_add_cursor:
	{
		...
		if (!data.is_selection) {
			...
			instr.textBuffer->ownedSelection_id.removeOrd(instr.cursor_id_index);
		}
	}break;
	}
