...
////   of C library functions used by stb_truetype.
   ...
   typedef unsigned short  stbtt_char16_t;
...
#define ttFixed(p)    ttLONG(p)
   ...
   #define ttUSHORT(p)   (* (stbtt_char16_t *) (p))
   ...
   stbtt_char16_t ttUSHORT(const stbtt_uint8 *p) { return p[0]*256 + p[1]; }
...
int stbtt_FindGlyphIndex(const stbtt_fontinfo *info, int unicode_codepoint)
{
   ...
   stbtt_char16_t format = ttUSHORT(data + index_map + 0);
   if (format == 0) { // apple byte encoding
      ...
      stbtt_char16_t segcount = ttUSHORT(data+index_map+6) >> 1;
      stbtt_char16_t searchRange = ttUSHORT(data+index_map+8) >> 1;
      stbtt_char16_t entrySelector = ttUSHORT(data+index_map+10);
      stbtt_char16_t rangeShift = ttUSHORT(data+index_map+12) >> 1;
      stbtt_char16_t item, offset, start, end;
      ...
      item = (stbtt_char16_t) ((search - endCount) >> 1);
      ...
      if (offset == 0)
         return (stbtt_char16_t) (unicode_codepoint + ttSHORT(data + index_map + 14 + segcount*4 + 2 + 2*item));
...
{
   stbtt_char16_t numOfLongHorMetrics = ttUSHORT(info->data+info->hhea + 34);
...
static stbtt_int32 stbtt__CompareUTF8toUTF16_bigendian_prefix(const stbtt_uint8 *s1, stbtt_int32 len1, const stbtt_uint8 *s2, stbtt_int32 len2) 
{
   ...
   while (len2) {
      stbtt_char16_t ch = s2[0]*256 + s2[1];
      if (ch < 0x80) {
         if (i >= len1) return -1;
         if (s1[i++] != ch) return -1;
      } else if (ch < 0x800) {
         ...
         stbtt_uint32 c;
         stbtt_char16_t ch2 = s2[2]*256 + s2[3];
